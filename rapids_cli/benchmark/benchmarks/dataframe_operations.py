...
def _prepare_join_data(n_left=5_000_000, n_right=2_000_000):
    ...
    left_data = {
        ...
        'category': np.random.choice(['grocery', 'entertainment', 'gas', 'retail'], n_left),
...
def _dataframe_join_operations_cpu(left_data, right_data, verbose=False):
    ...
    try:
        ...
        raise ImportError("pandas is required for this benchmark")
    ...
    inner_agg = inner_join.groupby(['category', 'segment', 'region']).agg({
    ...
    left_join['amount_bucket'] = pd.cut(left_join['amount'], bins=5, labels=['very_low', 'low', 'medium', 'high', 'very_high'])
    
    # Aggregations on left join
    left_agg = left_join.groupby(['category', 'region', 'has_profile']).agg({
        'amount': ['sum', 'mean', 'count'],
        'credit_score': 'mean',
    ...
    # Derived metrics
    inner_agg['avg_amount_per_loyalty_year'] = inner_agg['amount_mean'] / (inner_agg['loyalty_years_mean'] + 1)
    inner_agg['credit_score_range'] = inner_agg['credit_score_max'] - inner_agg['credit_score_min']
    
    # Final processing
    final_inner = inner_agg.sort_values(['amount_sum', 'credit_score_mean'], ascending=[False, False]).head(1000)
    final_left = left_agg.sort_values(['amount_sum', 'transaction_id_count'], ascending=[False, False]).head(1000)
    
    # Stop timing
    cpu_time = time.time() - start_time
    
    if verbose:
        print(f"Pandas completed in {cpu_time:.3f}s, results: {final_inner.shape}, {final_left.shape}")
...
def _dataframe_join_operations_gpu(left_data, right_data, verbose=False):
    ...
    try:
        ...
        raise ImportError("cudf is required for GPU benchmarking")
    ...
    if verbose:
        print("GPU warmup...")
    _ = left_df_gpu.head(100).merge(right_df_gpu.head(100), on='customer_id', how='inner')
    ...
    # Aggregations on inner join
    inner_agg_gpu = inner_join_gpu.groupby(['category', 'segment', 'region']).agg({
        'amount': ['sum', 'mean', 'count'],
        'age': 'mean',
        'income': ['median', 'std'],
        'credit_score': ['mean', 'min', 'max'],
    ...
    # Aggregations on left join
    left_agg_gpu = left_join_gpu.groupby(['category', 'region', 'has_profile']).agg({
        'amount': ['sum', 'mean', 'count'],
        'credit_score': 'mean',
        'transaction_id': 'count'
    }).reset_index()
    ...
    # Derived metrics
    inner_agg_gpu['avg_amount_per_loyalty_year'] = inner_agg_gpu['amount_mean'] / (inner_agg_gpu['loyalty_years_mean'] + 1)
    inner_agg_gpu['credit_score_range'] = inner_agg_gpu['credit_score_max'] - inner_agg_gpu['credit_score_min']
    
    # Final processing
    final_inner_gpu = inner_agg_gpu.sort_values(['amount_sum', 'credit_score_mean'], ascending=[False, False]).head(1000)
    final_left_gpu = left_agg_gpu.sort_values(['amount_sum', 'transaction_id_count'], ascending=[False, False]).head(1000)
    
    # Stop timing
    gpu_time = time.time() - start_time
    
    if verbose:
        print(f"cuDF computation completed in {gpu_time:.3f}s, results: {final_inner_gpu.shape}, {final_left_gpu.shape}")
...
def dataframe_join_operations(verbose: bool = False) -> tuple[float, float]:
    ...
    if verbose:
        print(f"Final comparison - CPU: {cpu_time:.3f}s, GPU: {gpu_time:.3f}s, Speedup: {cpu_time/gpu_time:.1f}x")
