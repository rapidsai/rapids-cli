...
def _fake_loader(overrides=None):
    ...
    defaults = {
        "cudart": FakeLoadedLib(abs_path="/usr/lib/libcudart.so.12"),
        "nvrtc": FakeLoadedLib(),
        "nvvm": FakeLoadedLib(),
    }
    if overrides:
...
def test_check_toolkit_newer_than_driver(tmp_path):
    ...
    cuda13_libs = {"cudart": FakeLoadedLib(abs_path="/usr/lib/libcudart.so.13")}
    with (
        patch(
            "cuda.pathfinder.load_nvidia_dynamic_lib",
            side_effect=_fake_loader(cuda13_libs),
        ),
        patch("pynvml.nvmlInit"),
        patch("pynvml.nvmlSystemGetCudaDriverVersion", return_value=12040),
        patch(
