...
def get_system_memory(VERBOSE_MODE=False):
...
def get_gpu_memory(VERBOSE_MODE=False):
    pynvml.nvmlInit()
...
def check_memory_to_gpu_ratio(VERBOSE_MODE=True):
    ...
    try:
        pynvml.nvmlInit()
        system_memory = get_system_memory(VERBOSE_MODE)
        gpu_memory = get_gpu_memory(VERBOSE_MODE)
